...
// 当前语言
const currentLanguage = ref('en'); // 默认语言为中文
// 表单数据
...
const currentImage = ref(imageList.value[1]); // 当前显示的图片
...
const handleLogin = async () => {
  ...
  if (xssPattern.test(loginForm.username) || xssPattern.test(loginForm.password)) {
    errorMessage('Invalid input. Please check your credentials.'); // 显示错误消息
    return // 停止执行
  }
  ...
  try {
    ...
    errorMessage('Login failed, please try again later.'); // 显示错误消息
  }
}
